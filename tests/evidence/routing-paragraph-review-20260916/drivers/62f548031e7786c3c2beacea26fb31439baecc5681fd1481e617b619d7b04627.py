...
CASES.update({'T10': '现有提纲是“第一节 阅读表现；第二节 阅读效率的提升”。已有正文第一节：24名参与者在平面图上指认东入口，18人正确、6人误指服务台。第二节：本次任务只安排图上指认，未记录实际行走和完成时间。请结合两节正文检查提纲是否对应，只给修订提纲，每节用一句话说明职责，不重写正文。', 'fresh_review': '请只审这两段课程论文，重点看整体结构、逻辑、重复和AI味，给简明意见，不代写。第一段的任务是报告结果，第二段是讨论。材料：某课程20人完成两次摘要练习，第二次有13人补齐了原文中的关键条件，7人仍有条件遗漏。没有记录改动原因，也没有测试迁移任务。原稿第一段：20人中，13人在第二次练习中补齐关键条件，7人仍有遗漏。第二段：从练习成效看，13人补齐条件，7人仍有遗漏。此次练习全面提高了学生的概括能力，学习机制日益完善。这不能说明全校学生都会受益，也不能证明所有任务都能完成，更不能说明长期能力得到改善。', 'empty_review': '请帮我审一下论文里这一段。'})

CASES.update({'review_result_guard': '请只审下面课程论文结果段，检查表述是否清楚、有无AI味和逻辑问题，给简短意见，不改稿。段落任务只是报告本次测试的描述性结果。已核对记录：18名参与者完成入口识别任务，其中15人选对入口，3人误选为服务台；本题不比较其他轮次。原文：本次入口识别任务中，18名参与者有15人选对入口，3人误选为服务台。'})
...
def main():
    ...
    binding['candidate_only'] = args.candidate_only
    ...
    def run_pair(index: int, case_id: str):
        results=[]
        for arm in (['candidate'] if args.candidate_only else ([baseline_arm,'candidate'] if index%2==0 else ['candidate',baseline_arm])):
