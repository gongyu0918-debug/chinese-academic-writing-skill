...
- 本地版本 0.0.8 依据公文 skill 1.5.40 已验证更新升级脚本与 AI 检测：终稿模式新增三类否定式句尾候选、意义支撑句尾成簇与正文外制作性注记标题检测；ANTI-AI 复核新增连续否定式收口与正向状态承接、纯检测模式输出约定；入口增加终稿 lint 短指针；ANTI-AI 首段加载条件减载为入口指针。真实写稿 A/B（4 任务双臂＋确认轮 T5/T6）未发现可复现候选独有硬失败，已决定任务 Candidate 3:2 Baseline；首轮两起候选独有失败经确认轮判定为双臂共有噪声。公开版本仍为 0.0.7，0.0.8 未发布。
