...
- 当前已完成独立入口、三条互斥专项叶、材料与引用门禁、成品残留终检，以及渐进加载的论文 ANTI-AI reference 和运行时只读 `prose_lint.py`；0.0.1 已完成 24 份真实写稿和 48 条匿名复核，0.0.2 另补脚本与渐进加载回归。
- 当前能力仍限定中文本科论文、硕士学位论文、课程论文、开题报告和独立文献综述。英文论文、投稿、统计分析、答辩、排版和检测规避尚未纳入。
- GitHub 完整仓库采用 MIT 许可证；ClawHub 与 skillhub.cn 的精简 Skill 包采用 MIT-0。运行目录只含 7 个必要文件，测试证据不进入 Skill 包。
...
6. 将详细 ANTI-AI 规则移至共用 reference，正文内容与证据稳定后才加载；新增从作者自有公文脚本适配的只读候选扫描器，排除公文文种、办理要素、项目卡片和算力规则。
